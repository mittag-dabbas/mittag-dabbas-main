...
const UserEmailTemplate: React.FC<{ cartData: any; userData: any; amount: any; quantity: any }> = ({
	cartData,
	userData,
	amount,
	quantity
					...
					<td style={{ padding: '20px', textAlign: 'left' }}>
						<div>
							<p style={{ margin: '5px 0', fontSize: '16px' }}>
								<strong>
									Hi, {userData.attributes.FirstName} {userData.attributes.LastName}
								</strong>
							</p>
									...
									>
										...
										<ul style={{ paddingLeft: '20px', marginBottom: '10px' }}>
											{day.items.map((item: any, index: number) => (
												...
												>
													{item.data.attributes.Name} - <strong>x{item.quantity}</strong> -{' '}
													<strong style={{ color: '#333' }}>
														€
														{(item.data.attributes.OfferedPrice * item.quantity).toFixed(2)}
													</strong>
												</li>
											))}
											<br />
