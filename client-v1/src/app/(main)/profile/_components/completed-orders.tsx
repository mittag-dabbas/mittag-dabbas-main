...
const CompletedOrders: React.FC<CompletedOrdersProps> = ({ tabValue }) => {
	...
	const fetchOrders = async () => {
		try {
			...
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_STRAPI_API_BASE_URL}/orders?populate[MenuItems][populate]=ItemImage&populate=*&filters[UiD][$eq]=${user.uid}&filters[isOrderCompleted][$eq]=${true}&filters[isOrderCancelled][$eq]=${false}&sort=updatedAt:desc`,
				{
					method: 'GET',
	...
	if (!orders || orders.length === 0) {
		return (
			...
			>
				<Typography variant='h6'>No orders have been completed!</Typography>
				<Button
					variant='contained'
					size='large'
					sx={{ px: 5, mt: 2 }}
